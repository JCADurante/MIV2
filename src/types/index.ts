...
export type StorageMode = 'LOCAL' | 'SHARED_NETWORK';

export interface DatabaseHealthIssue {
  id: string;
  type: 'LOCK' | 'TIMEOUT' | 'CORRUPTION' | 'PERMISSION' | 'WARNING';
...
export interface DatabaseHealthReport {
  lastScanned: string;
  overallHealth: 'HEALTHY' | 'DEGRADED' | 'CORRUPTED' | 'LOCKED' | 'UNREACHABLE';
  sharedFolderAccessible: boolean;
  fileLockStatus: 'NO_LOCKS' | 'LOCK_DETECTED' | 'STALE_LOCK_FOUND';
  connectionTimeoutMs: number;
  dbIntegrityStatus: 'VALID' | 'CORRUPTED' | 'MISSING' | 'INVALID_JSON';
  recordCount: number;
  dbFileSizeKb: number;
  lastWriterWorkstation?: string;
  lastWriterUser?: string;
  lastUpdatedTime?: string;
  issues: DatabaseHealthIssue[];
}

export interface SharedPathCheckResult {
  lastChecked: string;
  syntaxValid: boolean;
  folderAccessible: boolean;
  canRead: boolean;
  canWrite: boolean;
  sharedDbFound: boolean;
  sharedDbRecordCount?: number;
  sharedDbLastUpdated?: string;
  lastUpdatedByPC?: string;
  status: 'CONNECTED' | 'READ_ONLY' | 'ERROR' | 'UNCHECKED';
  errorMessage?: string;
}

export interface AppConfig {
  appName: string;
  storageMode?: StorageMode; // 'LOCAL' (beside EXE) or 'SHARED_NETWORK' (network path)
  dataDirectory: string; // Local path beside EXE: "Application Data/ReferenceTracker_Data/" or ".\ReferenceTracker_Data\"
  sharedFolderPath?: string; // e.g. "\\192.168.1.100\SharedData\" or "Z:\MIRMS_Data\"
  ...
  lastSharedSyncTime?: string;
  lastSharedSyncStatus?: 'CONNECTED' | 'READ_ONLY' | 'ERROR' | 'SYNCING' | 'DISCONNECTED' | 'UNCHECKED';
  sharedPathCheckResult?: SharedPathCheckResult;
