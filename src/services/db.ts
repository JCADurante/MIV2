...
class DatabaseService {
  ...
  public async init(): Promise<void> {
    ...
    if (this.config.storageMode === 'SHARED_NETWORK' || (this.config.sharedFolderSyncEnabled && this.config.sharedFolderPath)) {
      this.pullFromSharedFolderNetwork(true).catch((e) => console.warn('Initial shared network pull warning:', e));
    }
    this.notifyListeners();
  }
  ...
  private sharedSyncDebounceTimer: any = null;
  private sharedSyncPollingTimer: any = null;
  ...
  public async syncToSharedFolderNetwork(): Promise<{ success: boolean; message?: string }> {
    const isSharedActive = this.config.storageMode === 'SHARED_NETWORK' || (this.config.sharedFolderSyncEnabled && this.config.sharedFolderPath);
    if (!isSharedActive || !this.config.sharedFolderPath) {
      return { success: false, message: 'Shared network folder mode is disabled or no path is set.' };
    }
    ...
    try {
      const payload = {
        version: '2.0',
        updatedAt: new Date().toISOString(),
        updatedByWorkstation: this.config.workstationName || realtimeSync.getCurrentUser().workstationName || 'PC-Workstation',
        updatedByUser: this.config.defaultRegisteredBy || 'QC Inspector',
        masterItems: this.masterItems,
        registrations: this.registrations,
        auditLogs: this.auditLogs,
        formTemplates: this.formTemplates,
      ...
      const success = await tauriBridge.writeSharedDatabaseFile(this.config.sharedFolderPath, payload);
      if (success) {
        this.lastSharedSyncTimestamp = payload.updatedAt;
        this.config = {
          ...this.config,
          lastSharedSyncTime: payload.updatedAt,
          lastSharedSyncStatus: 'CONNECTED'
        };
        this.safeSetItem(STORAGE_KEYS.APP_CONFIG, JSON.stringify(this.config));
        return { success: true, message: `Successfully saved shared database to network path "${this.config.sharedFolderPath}".` };
      } else {
        this.config = { ...this.config, lastSharedSyncStatus: 'ERROR' };
        return { success: false, message: `Failed to write shared database file to network path "${this.config.sharedFolderPath}". Check folder write permissions.` };
      }
    } catch (err: any) {
      this.config = { ...this.config, lastSharedSyncStatus: 'ERROR' };
      return { success: false, message: `Shared network write error: ${err?.message || 'Access Denied'}` };
    }
  }
  ...
  public async pullFromSharedFolderNetwork(force = false): Promise<{ success: boolean; updated: boolean; message: string }> {
    const isSharedActive = this.config.storageMode === 'SHARED_NETWORK' || (this.config.sharedFolderSyncEnabled && this.config.sharedFolderPath);
    if (!isSharedActive || !this.config.sharedFolderPath) {
      return { success: false, updated: false, message: 'Shared network folder mode is disabled.' };
    }

    if (this.isSyncingSharedNetwork) return { success: true, updated: false, message: 'Sync in progress' };
    ...
    try {
      ...
      if (force || (isFromOtherPC && isNewer) || (!meta && force)) {
        ...
        if (sharedDb && Array.isArray(sharedDb.masterItems)) {
          ...
          if (Array.isArray(sharedDb.registrations)) this.registrations = sharedDb.registrations;
          if (Array.isArray(sharedDb.auditLogs)) this.auditLogs = sharedDb.auditLogs;
          if (Array.isArray(sharedDb.formTemplates) && sharedDb.formTemplates.length > 0) {
      ...
      this.isSyncingSharedNetwork = false;
      return { success: true, updated: false, message: 'Data is synchronized with shared folder.' };
    } catch (err: any) {
      this.isSyncingSharedNetwork = false;
      this.config = { ...this.config, lastSharedSyncStatus: 'ERROR' };
      return { success: false, updated: false, message: `Failed to read from shared network folder: ${err?.message || 'Unreachable'}` };
    }
  }
  ...
  public startSharedNetworkPolling(): void {
    ...
    this.sharedSyncPollingTimer = setInterval(() => {
      const isSharedActive = this.config.storageMode === 'SHARED_NETWORK' || (this.config.sharedFolderSyncEnabled && this.config.sharedFolderPath);
      if (isSharedActive) {
        this.pullFromSharedFolderNetwork(false);
      }
    }, Math.max(1, intervalSec) * 1000);
  }

  /**
   * Run multi-check diagnostic test on a shared folder path.
   */
  public async checkSharedFolderPath(targetPath: string): Promise<any> {
    const result = await tauriBridge.testSharedPathAccess(targetPath);
    this.config = {
      ...this.config,
      sharedPathCheckResult: result,
      lastSharedSyncStatus: result.status
    };
    this.saveLocalConfig();
    return result;
  }
  ...
  private saveMasterItems(): void {
    this.safeSetItem(STORAGE_KEYS.MASTER_ITEMS, JSON.stringify(this.masterItems));
    this.triggerSharedNetworkAutoSave();
  }
