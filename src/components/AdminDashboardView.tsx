...
import { SharedNetworkStoragePanel } from './SharedNetworkStoragePanel';
...
export const AdminDashboardView: React.FC<AdminDashboardViewProps> = ({
  ...
  const [activeAdminSubTab, setActiveAdminSubTab] = useState<
    'OVERVIEW' | 'APPROVAL_QUEUE' | 'DASHBOARD_VISIBILITY' | 'CATEGORIES' | 'FORM_TEMPLATES' | 'SHARED_SYNC' | 'DB_HEALTH' | 'EXCEL' | 'CUSTOM_FIELDS' | 'AUDIT' | 'BACKUP' | 'SETTINGS' | 'USER_ACCESS'
  >(
    initialSubTab === 'APPROVAL_QUEUE'
      ...
      ? 'SHARED_SYNC'
      : initialSubTab === 'DB_HEALTH' || initialSubTab === 'DATABASE_HEALTH'
      ? 'DB_HEALTH'
  ...
  // Handle auto-saving fields on input blur or immediate selection change
  const handleAutoSave = async (updatedFields: Partial<AppConfig>) => {
    try {
      await onConfigChange(updatedFields);
      if (updatedFields.workstationName !== undefined || updatedFields.defaultRegisteredBy !== undefined) {
        realtimeSync.updateUserInfo(
          updatedFields.workstationName ?? workstationName,
          updatedFields.defaultRegisteredBy ?? defaultInspector
        );
      }
    } catch (err: any) {
  ...
  const handleDeleteUser = (idNumber: string) => {
    if (window.confirm(`Are you sure you want to delete user ${idNumber}?`)) {
      const success = userService.deleteUser(idNumber);
      if (success) {
        setStatusMessage({ type: 'success', text: `User ${idNumber} deleted.` });
      } else {
        setStatusMessage({ type: 'error', text: `Failed to delete user ${idNumber}.` });
      }
    }
  };

  // Handle open shared directory
  ...
  const handleRestoreFactorySettings = async () => {
    if (!window.confirm('CRITICAL WARNING: This will permanently wipe all current data and restore the system to its initial factory settings. Are you absolutely sure?')) return;
    setIsRestoring(true);
    try {
      ...
      setStatusMessage({ type: 'error', text: `Failed to restore factory settings: ${err?.message}` });
    } finally {
      setIsRestoring(false);
    }
  };
  ...
  return (
    <div className="space-y-6 select-none font-sans pb-12">
      ...
      <div className="flex items-center gap-1.5 border-b border-[#222] pb-3 overflow-x-auto select-none">
        ...
        <button
          onClick={() => setActiveAdminSubTab('DB_HEALTH')}
          className={`flex items-center gap-2 px-4 py-2 text-xs font-bold rounded-xl transition-all whitespace-nowrap ${
            activeAdminSubTab === 'DB_HEALTH'
              ? 'bg-purple-600 text-white shadow-sm'
              : 'bg-[#141414] text-gray-400 hover:text-white hover:bg-[#1C1C1C] border border-[#222]'
          }`}
        >
          <Activity className="w-3.5 h-3.5 text-purple-400" />
          <span>Database Health Diagnostic</span>
        </button>
      ...
      {/* SUB-SECTION: DATABASE HEALTH & INTEGRITY DIAGNOSTIC CENTER */}
      {activeAdminSubTab === 'DB_HEALTH' && (
        <div className="space-y-6 animate-in fade-in duration-150">
          ...
          <div className="flex items-center justify-between pb-4 border-b border-[#222]">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                disabled={isRestoring}
                className="px-4 py-2 bg-red-500/10 hover:bg-red-500/20 border border-red-500/30 text-red-400 text-xs font-bold rounded-xl transition-all flex items-center gap-2 disabled:opacity-50 ml-auto"
              >
                <RefreshCw className="w-4 h-4" />
                <span>{isRestoring ? 'Restoring...' : 'Restore Factory Settings'}</span>
              </button>
            </div>
          </div>
          ...
          <form onSubmit={handleSaveSettings} className="space-y-5 max-w-2xl">
            <div className="space-y-1.5">
              ...
              <input
                ...
                onBlur={() => handleAutoSave({ companyName: companyName.trim() })}
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                onBlur={() => handleAutoSave({ defaultRegisteredBy: defaultInspector.trim() })}
                className="w-full px-3.5 py-2 text-xs bg-[#1C1C1C] border border-[#333] text-white rounded-xl focus:outline-hidden focus:border-blue-500 font-medium"
              />
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                onBlur={() => handleAutoSave({ workstationName: workstationName.trim() })}
                className="w-full px-3.5 py-2 text-xs bg-[#1C1C1C] border border-[#333] text-white rounded-xl focus:outline-hidden focus:border-blue-500 font-medium"
              />
            </div>
